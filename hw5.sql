...
SELECT u.firstname,u.lastname, p.gender,p.hometown
FROM users AS u JOIN profiles AS p ON
u.id = p.user_id
WHERE (YEAR (NOW())-YEAR(p.birthday)) <= 20;

SELECT * FROM users_info;

/*2. Найдите кол-во, отправленных сообщений 
каждым пользователем и выведите ранжированный 
список пользователей, указав имя и фамилию пользователя, 
...
SELECT 
COUNT(m.id) AS number, CONCAT(u.firstname, ' ', u.lastname) AS user,
DENSE_RANK () 
OVER (ORDER BY COUNT(m.id) DESC ) AS 'rank'
FROM users AS u JOIN messages AS m ON
u.id = m.from_user_id
GROUP BY user
ORDER BY number DESC;

/* 3.Выберите все сообщения, отсортируйте сообщения по возрастанию даты отправления (created_at) и 
найдите разницу дат отправления между соседними сообщениями, 
получившегося списка. (используйте LEAD или LAG)*/

#в днях
SELECT id,
LEAD (created_at,1,0) OVER (ORDER BY created_at) AS date_creation,
DATEDIFF(created_at, LEAD(created_at) OVER(ORDER BY created_at)) AS difference_in_days
FROM messages;

#в минутах
SELECT id,
LEAD (created_at,1,0) OVER (ORDER BY created_at) AS date_creation,
TIMESTAMPDIFF(minute,created_at,LEAD(created_at) OVER(ORDER BY created_at)) AS difference_in_min
FROM messages;
